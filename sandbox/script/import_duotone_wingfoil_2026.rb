...
# Duotone Wing Foiling 2026 import from Google Sheets CSV (Shopify format)
#
...
#
# Run on server:
#   RAILS_ENV=production bundle exec rails runner script/import_duotone_wingfoil_2026.rb
#
# Or via Kamal:
#   bin/kamal app exec --reuse "bin/rails runner /rails/script/import_duotone_wingfoil_2026.rb"
...
CURRENCY = 'EUR'
STORE_ID = 2
CSV_URL  = 'https://docs.google.com/spreadsheets/d/1OXb4No4pzBs8hwbMB7q17jMV17HTV37Nl7kwagn2OjY/export?format=csv'

# Product Category / category_level_2 tag → Spree taxon permalink
CATEGORY_MAP = {
  'Wings'              => 'categories/wingfoil/wings',
  'Wing'               => 'categories/wingfoil/wings',
  'Parawings'          => 'categories/kitesurfing/kites/parawings',
  ...
  'Foil'               => 'categories/wingfoil/wing-foils/foil',
  'Foil Wing Set'      => 'categories/wingfoil/wing-foils/wing-foil-wing-sets',
  'Foilparts'          => 'categories/wingfoil/wing-foils/wing-foil-parts',
  'Fusepart'           => 'categories/wingfoil/wing-foils/wing-foil-parts',
  'Mast'               => 'categories/wingfoil/wing-foils/wing-foil-parts',
  'Boards'             => 'categories/wingfoil/wing-boards',
  'Foilboard'          => 'categories/wingfoil/wing-boards',
  'iFoilboard'         => 'categories/wingfoil/wing-boards',
  'Accessories'        => 'categories/wingfoil/wing-accessories',
...
rows_by_handle.each_with_index do |(handle, rows), idx|
  ...
  # Skip if ANY barcode/EAN already exists in the DB
  ...
  # Determine sub-category from Product Category column or tags
  product_cat = first['Product Category'].to_s.strip
  tags_str    = first['Tags'].to_s

  category_taxon = category_taxons[product_cat]

  # If not found by Product Category, try category_level_2 tag
  unless category_taxon
    CATEGORY_MAP.each_key do |hint|
  ...
  # Build slug — append -2026, avoid collisions
  ...
  begin
    ActiveRecord::Base.transaction do
      ...
      # Also attach variant-specific images if different from product images
      rows.each do |row|
        variant_img = row['Variant Image']
        if variant_img.present? && !attached_images[variant_img]
          ok = attach_image(product.master, variant_img)
...
puts "\n\n#{'=' * 60}"
puts "Duotone Wing Foiling 2026 import complete in #{elapsed}s"
