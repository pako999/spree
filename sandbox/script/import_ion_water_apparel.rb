...
# ION Water apparel import from Google Sheets CSV (Shopify format)
#
# Prices in the CSV are stored as integer × 10 (e.g. 299 = €29.90).
# Skip any product whose ANY variant barcode already exists in the DB.
#
# Run:
#   bin/kamal app exec --reuse "bin/rails runner /rails/script/import_ion_water_apparel.rb"
...
CURRENCY = 'EUR'
CSV_URL  = 'https://docs.google.com/spreadsheets/d/1XjeyXrBhGRLMy8sMDBBhrzXvS464kPYkaaiVS1pURfA/export?format=csv'

# Shopify category_level_2 tag → Spree taxon permalink
CATEGORY_TAG_MAP = {
  'Beanie & Cap'    => 'categories/apparel/cap',
  'Boardshorts'     => 'categories/apparel/boardshorts',
  'Tee'             => 'categories/apparel/tops',
  'Shirt'           => 'categories/apparel/tops',
  'Hoody & Sweater' => 'categories/apparel/tops',
  'Jacket'          => 'categories/apparel/coats',
  'Socks'           => 'categories/apparel/tops',
  'Shorts'          => 'categories/apparel/boardshorts',
  'Pants'           => 'categories/apparel/coats',
}.freeze
...
brand_taxon    = Spree::Taxon.find_by!(permalink: 'brands/ion-water')
...
rows_by_handle.each_with_index do |(handle, rows), idx|
  ...
  if barcodes.any? && Spree::Variant.with_deleted.where(barcode: barcodes).exists?
    puts "\n[#{idx + 1}/#{total}] SKIP #{title} (barcode exists)"
    skipped += 1
    next
  end
  ...
  # Prices in CSV are × 10 (e.g. 299 = €29.90)
  base_price = first['Variant Price'].to_f / 10.0
  ...
  begin
    ActiveRecord::Base.transaction do
      ...
      # Pre-attach all unique product images to master variant
      ...
      rows.each do |row|
        ...
        price     = row['Variant Price'].to_f / 10.0
...
puts "\n\n#{'=' * 60}"
puts "ION Water apparel import complete in #{elapsed}s"
