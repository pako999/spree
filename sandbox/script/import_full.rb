...
# Full ION product import from Google Sheets CSV
# Run on production: bin/kamal app exec --reuse "bin/rails runner /rails/script/import_full.rb"
# Or locally against production DB by SSH + docker exec
...
# Download image via Net::HTTP following redirects; returns Tempfile or nil
...
puts "Shipping category: #{shipping_category.name}"
...
rows_by_handle.each_with_index do |(handle, rows), idx|
  ...
  if Spree::Variant.with_deleted.where(sku: skus).exists?
    print "."
    skipped += 1
    next
  end
  ...
  slug = handle.sub(/-\d{4}$/, '')
  slug = "#{slug}-ion" if Spree::Product.with_deleted.find_by(slug: slug)
...
puts "  Skipped  : #{skipped} (SKU already exists)"
puts "  Errors   : #{errors}"
puts "  Total    : #{total}"
